...
export const AttachmentsForm = ({
    ...
    const getFilenameFromUrl = (url: string): string => {
        try {
            ...
            if (filename) {
                ...
                return cleanFilename || t("teacher.chapterEdit.chapterDocument");
            }
            return t("teacher.chapterEdit.chapterDocument");
        } catch {
            return t("teacher.chapterEdit.chapterDocument");
        }
    };
    ...
    const downloadDocument = async (url: string, name: string) => {
        try {
            ...
            if (response.ok) {
                ...
                toast.success(t("teacher.chapterEdit.downloadStarted"));
            } else {
            ...
            toast.success(t("teacher.chapterEdit.downloadOpened"));
        }
    };
    ...
    const onSubmitUpload = async (url: string, name: string) => {
        try {
            ...
            toast.success(t("teacher.chapterEdit.documentUploaded"));
        } catch (error) {
    ...
    const onDelete = async (attachmentId: string) => {
        try {
            ...
            toast.success(t("teacher.chapterEdit.documentDeleted"));
            router.refresh();
        } catch (error) {
            console.error("[CHAPTER_ATTACHMENT_DELETE]", error);
            const errorMessage = error instanceof Error ? error.message : t("teacher.chapterEdit.updateError");
                ...
                <Button onClick={() => setIsEditing(!isEditing)} variant="ghost">
                    {isEditing ? (
                        ...
                        <>
                            ...
                            {t("teacher.chapterEdit.manageDocuments")}
                        </>
                    )}
                </Button>
            </div>
                ...
                <div className="mt-2">
                    {attachments.length > 0 ? (
                            ...
                            {attachments.map((attachment) => (
                                <div key={attachment.id} className="flex items-center p-3 w-full bg-secondary/50 border-secondary/50 border text-secondary-foreground rounded-md">
                                    ...
                                    <div className="flex flex-col min-w-0 flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">{t("teacher.chapterEdit.chapterDocument")}</p>
                                    </div>
                                    <div className="ml-auto flex items-center gap-2 flex-shrink-0">
                                        ...
                                        >
                                            ...
                                            {t("teacher.chapterEdit.download")}
                                        </Button>
                                    </div>
                                </div>
                            ))}
                        ...
                        <p className="text-sm mt-2 text-muted-foreground italic">
                            {t("teacher.chapterEdit.noDocuments")}
                        </p>
                    )}
                </div>
            )}
                ...
                <div className="mt-4 space-y-4">
                    ...
                    <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-4">
                        ...
                        <div className="text-xs text-muted-foreground mt-2 text-center">
                            {t("teacher.chapterEdit.addDocumentsHint")}
                        </div>
                    </div>
                </div>
            )}
