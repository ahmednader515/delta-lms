...
export const VideoForm = ({
    ...
    const onSubmitYouTube = async () => {
        if (!youtubeUrl.trim()) {
            toast.error(t("teacher.chapterEdit.youtubeError"));
            return;
        }
        ...
        try {
            ...
            toast.success(t("teacher.chapterEdit.youtubeSuccess"));
            setIsEditing(false);
            setYoutubeUrl("");
            router.refresh();
        } catch (error) {
                    ...
                    <Tabs defaultValue="upload" className="w-full">
                        <TabsList className="grid w-full grid-cols-2">
                            <TabsTrigger value="upload" className="flex items-center gap-2">
                                ...
                                {t("teacher.chapterEdit.uploadVideo")}
                            </TabsTrigger>
                            <TabsTrigger value="youtube" className="flex items-center gap-2">
                                <Youtube className="h-4 w-4" />
                                {t("teacher.chapterEdit.youtubeLink")}
                            </TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="upload" className="mt-4">
                            <div className="space-y-4">
                                <div className="text-sm text-muted-foreground">
                                    {t("teacher.chapterEdit.uploadFromDevice")}
                                </div>
                                <FileUpload
                        ...
                        <TabsContent value="youtube" className="mt-4">
                            <div className="space-y-4">
                                <div className="text-sm text-muted-foreground">
                                    {t("teacher.chapterEdit.pasteYoutubeLink")}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="youtube-url">{t("teacher.chapterEdit.youtubeUrl")}</Label>
                                ...
                                <div className="text-xs text-muted-foreground">
                                    {t("teacher.chapterEdit.supportedLinks")}
                                    <br />
