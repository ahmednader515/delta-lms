...
const ProgressPage = () => {
    ...
    const fetchUsers = async () => {
        try {
            setLoading(true);
            console.log("[PROGRESS_PAGE] Fetching users...");
            const response = await fetch("/api/teacher/users");
            console.log("[PROGRESS_PAGE] Response status:", response.status, response.statusText);
            
            if (response.ok) {
                const data = await response.json();
                console.log("[PROGRESS_PAGE] Users data received:", data);
                setUsers(data || []);
            } else {
                const errorText = await response.text();
                console.error("[PROGRESS_PAGE] Error fetching users:", response.status, response.statusText, errorText);
                setUsers([]);
            }
        } catch (error) {
            console.error("[PROGRESS_PAGE] Error fetching users:", error);
            setUsers([]);
        } finally {
            setLoading(false);
        }
                                            ...
                                            {userPurchases.length === 0 ? (
                                                <TableRow>
                                                    <TableCell colSpan={4} className="text-center text-muted-foreground">
                                                        {t("admin.progress.dialog.purchasedCourses.noPurchases")}
                                                    </TableCell>
                                                </TableRow>
                                            ) : (
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {allChapters.length === 0 ? (
                                                <TableRow>
                                                    <TableCell colSpan={4} className="text-center text-muted-foreground">
                                                        {t("admin.progress.dialog.progressDetails.noChapters")}
                                                    </TableCell>
                                                </TableRow>
                                            ) : (
